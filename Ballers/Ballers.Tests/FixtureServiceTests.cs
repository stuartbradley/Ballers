...
namespace Ballers.Tests
{
    public class FixtureServiceTests
    {
        ...
        [Fact]
        public async Task UpdateSchedule_SavesPostcode()
        {
            var db = await SeedBase(nameof(UpdateSchedule_SavesPostcode));
            db.Fixtures.Add(MakeFixture(1));
            await db.SaveChangesAsync();
            ...
            var fixture = db.Fixtures.Find(1)!;
            Assert.Equal("FY1 6JJ", fixture.Postcode);
        }

        // ── SaveCaptaincyAsync ──────────────────────────────────────────

        [Fact]
        public async Task SaveCaptaincy_HomeTeam_SetsCaptainAndVice()
        {
            var db = await SeedBase(nameof(SaveCaptaincy_HomeTeam_SetsCaptainAndVice));
            db.Fixtures.Add(MakeFixture(1, homeId: 1, awayId: 2));
            db.Players.AddRange(
                new Player { Id = 10, Name = "Captain", TeamId = 1 },
            ...
            var fixture = db.Fixtures.Find(1)!;
            Assert.Equal(10, fixture.HomeCaptainId);
            Assert.Equal(11, fixture.HomeViceCaptainId);
            Assert.Null(fixture.AwayCaptainId);
            Assert.Null(fixture.AwayViceCaptainId);
        }

        [Fact]
        public async Task SaveCaptaincy_AwayTeam_SetsCaptainAndVice()
        {
            var db = await SeedBase(nameof(SaveCaptaincy_AwayTeam_SetsCaptainAndVice));
            db.Fixtures.Add(MakeFixture(1, homeId: 1, awayId: 2));
            db.Players.AddRange(
                new Player { Id = 20, Name = "AwayCap", TeamId = 2 },
            ...
            var fixture = db.Fixtures.Find(1)!;
            Assert.Equal(20, fixture.AwayCaptainId);
            Assert.Equal(21, fixture.AwayViceCaptainId);
            Assert.Null(fixture.HomeCaptainId);
            Assert.Null(fixture.HomeViceCaptainId);
        }

        [Fact]
        public async Task SaveCaptaincy_NullIds_ClearsCaptaincy()
        {
            var db = await SeedBase(nameof(SaveCaptaincy_NullIds_ClearsCaptaincy));
            var fixture = MakeFixture(1, homeId: 1, awayId: 2);
            fixture.HomeCaptainId = 5;
            fixture.HomeViceCaptainId = 6;
            ...
            Assert.Null(updated.HomeCaptainId);
            Assert.Null(updated.HomeViceCaptainId);
        }
        ...
        [Fact]
        public async Task GetHeadToHead_ReturnsMatchesBetweenBothOrders()
        {
            var db = await SeedBase(nameof(GetHeadToHead_ReturnsMatchesBetweenBothOrders));
            db.Fixtures.AddRange(
                new Fixture { Id = 10, HomeTeamId = 1, AwayTeamId = 2, SeasonId = 1, IsPlayed = true, HomeScore = 3, AwayScore = 1, WindowStart = DateTime.UtcNow, WindowEnd = DateTime.UtcNow },
        ...
        [Fact]
        public async Task GetHeadToHead_ExcludesSpecifiedFixture()
        {
            var db = await SeedBase(nameof(GetHeadToHead_ExcludesSpecifiedFixture));
            db.Fixtures.AddRange(
                new Fixture { Id = 10, HomeTeamId = 1, AwayTeamId = 2, SeasonId = 1, IsPlayed = true, HomeScore = 1, AwayScore = 0, WindowStart = DateTime.UtcNow, WindowEnd = DateTime.UtcNow },
            ...
            Assert.Single(result);
            Assert.Equal(11, result[0].FixtureId);
        }

        [Fact]
        public async Task GetHeadToHead_OnlyIncludesPlayedFixtures()
        {
            var db = await SeedBase(nameof(GetHeadToHead_OnlyIncludesPlayedFixtures));
            db.Fixtures.AddRange(
                new Fixture { Id = 10, HomeTeamId = 1, AwayTeamId = 2, SeasonId = 1, IsPlayed = true,  HomeScore = 1, AwayScore = 0, WindowStart = DateTime.UtcNow, WindowEnd = DateTime.UtcNow },
        ...
        [Fact]
        public async Task GetHeadToHead_ExcludesUnrelatedFixtures()
        {
            var db = await SeedBase(nameof(GetHeadToHead_ExcludesUnrelatedFixtures));
            db.Teams.Add(new Team { Id = 3, Name = "Third FC" });
            db.Fixtures.AddRange(
                new Fixture { Id = 10, HomeTeamId = 1, AwayTeamId = 2, SeasonId = 1, IsPlayed = true, HomeScore = 1, AwayScore = 0, WindowStart = DateTime.UtcNow, WindowEnd = DateTime.UtcNow },
            ...
            Assert.Single(result);
            Assert.Equal(10, result[0].FixtureId);
        }

        [Fact]
        public async Task GetHeadToHead_LimitedToTenResults()
        {
            var db = await SeedBase(nameof(GetHeadToHead_LimitedToTenResults));
            for (int i = 1; i <= 12; i++)
                db.Fixtures.Add(new Fixture
                {
                    Id = i, HomeTeamId = 1, AwayTeamId = 2, SeasonId = 1,
                    IsPlayed = true, HomeScore = 1, AwayScore = 0,
                    WindowStart = DateTime.UtcNow, WindowEnd = DateTime.UtcNow,
                    Kickoff = DateTime.UtcNow.AddDays(-i)
                });
            await db.SaveChangesAsync();
