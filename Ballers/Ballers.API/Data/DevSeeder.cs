...
namespace Ballers.API.Data
{
    public static class DevSeeder
    {
        ...
        public static async Task SeedNewSeasonAsync(ApplicationDbContext db, int seasonNumber)
        {
            var current = await db.Seasons.FirstOrDefaultAsync(s => s.IsActive);
            if (current != null)
                current.IsActive = false;

            var teams = await db.Teams.ToListAsync();
            if (teams.Count == 0) throw new InvalidOperationException("No teams found.");
            ...
            var rng = new Random(42 + seasonNumber);
            ...
            foreach (var (home, away, round) in allMatchups)
            {
                ...
                var kickoff     = windowStart.AddDays(6).Date
                                  + TimeSpan.FromHours(KickoffHours[rng.Next(KickoffHours.Length)]);
